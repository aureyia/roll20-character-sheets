on("clicked:momentum-burn", function(eventInfo) {
  ...
  getAttrs(["momentum_reset", "momentum"], function(rollData) {
    ...
    if (momentum > momentum_reset) {
      const template = `&{template:ironsworn_move} {{surtitle=@{character_name}}} {{header=Burn Momentum}} {{action-score=[[@{momentum}]]}} {{rollId=0}} {{customLabel1=Momentum reset}} {{customRow1=[[@{momentum_reset}]]}} `;
